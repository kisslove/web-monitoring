...
export class VisitOsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.unsubscribe.sub1 = this.broadcaster.on("choiceTimeToRender").subscribe((data: any) => {
      this.loadTerminalList(data.time, data.type);
    });
    if (window.globalTime) {
      this.loadTerminalList(window.globalTime.time, window.globalTime.type);
    } else {
      this.loadTerminalList(null, 4);
    }
  ...
  nzSelectedIndexChange(d) {
    ...
    if (this.currentSelectedTab == 0) {
      this.currentSelectedTabType = 31;
    }
    if (this.currentSelectedTab == 1) {
      this.currentSelectedTabType = 32;
    }
    if (this.currentSelectedTab == 2) {
      this.currentSelectedTabType = 33;
    }
    if (window.globalTime) {
      this.loadTerminalList(window.globalTime.time, window.globalTime.type);
    } else {
  ...
  //获取终端列表
  loadTerminalList(time, type) {
    this.isSpinning.spin1 = true;
    this.http.post("Monitor/TerminalListStatis", {
      TimeQuantum: type == '7' ? '' : type,
      sTime: type == '7' ? time[0] : '',
      eTime: type == '7' ? time[1] : '',
      appKey: this.appKey,
      ternimalType: this.currentSelectedTab
    }).subscribe((d: any) => {
      if (d.IsSuccess) {
        if (d.Data && d.Data.length > 0) {
          d.Data[0]['select'] = true;
          this.terminalListData = d.Data;
          this.selectTerminalListItem(d.Data[0]);
        } else {
  ...
  selectTerminalListItem(data) {
    _.each(this.terminalListData, (val) => {
      val.select = false;
    });
    data.select = true;
    this.currentSelectedTerminal = data.terminal;
    if (window.globalTime) {
  ...
  loadVisitSpeedData(time, type) {
    ...
    this.http.post("Monitor/VisitSpeedStatic", {
      ...
      eTime: type == '7' ? time[1] : '',
      appKey: this.appKey,
      keywords: this.currentSelectedTerminal,
      kerfType: this.currentSelectedTabType
    }).subscribe((d: any) => {
      if (d.IsSuccess) {
  ...
  loadJsData(time, type) {
    ...
    this.http.post("Monitor/JsErrorRate", {
      ...
      appKey: this.appKey,
      keywords: this.currentSelectedTerminal,
      errorRateType: this.currentSelectedTabType
    }).subscribe((d: any) => {
      if (d.IsSuccess) {
  ...
  loadApiSuccRateData(time, type) {
    ...
    this.http.post("Monitor/ApiSuccRateStatic", {
      ...
      appKey: this.appKey,
      keywords: this.currentSelectedTerminal,
      apiSuccRateType: this.currentSelectedTabType
    }).subscribe((d: any) => {
      if (d.IsSuccess) {
  ...
  private _resizePageHeight() {
    this.mydetailsContent.forEach(element => {
      this.render.setStyle(element.nativeElement, "height", window.innerHeight - 50 - 70-25 + "px");
    });
